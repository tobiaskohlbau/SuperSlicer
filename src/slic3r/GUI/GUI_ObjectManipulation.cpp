...
void ObjectManipulation::Show(const bool show)
{
    if (show != IsShown()) {
        ...
        if (show && (wxGetApp().get_mode() != comSimple || get_app_config()->get_bool("objects_always_expert"))) {
    ...
    if (show) {
        ...
        bool show_world_local_combo = (wxGetApp().get_mode() != comSimple || get_app_config()->get_bool("objects_always_expert"))
...
//   ImGui::SetNextWindowCollapsed(true, ImGuiCond_Once);
    ...
    imgui.text_colored(ImGuiWrapper::get_COL_LIGHT(), "Coordinates type");
    ImGui::SameLine();
