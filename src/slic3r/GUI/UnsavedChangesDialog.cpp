...
//    m_action_line->SetFont(wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT).Bold());
    ...
    if (ActionButtons::TRANSFER & m_buttons) {
        ...
        if (dependent_presets)
            new_preset = (type == dependent_presets->type() ? dependent_presets->find_preset(new_selected_preset) : switched_presets->find_preset(new_selected_preset));
        if (new_preset && dependent_presets && switched_presets && (type == dependent_presets->type() ?
            dependent_presets->get_edited_preset().printer_technology() == new_preset->printer_technology() :
            switched_presets->get_edited_preset().printer_technology() == new_preset->printer_technology()))
...
void DiffPresetDialog::update_compatibility(const std::string& preset_name, Preset::Type type, PresetBundle* preset_bundle)
{
    ...
    bool print_tab = type == Preset::TYPE_FFF_PRINT || type == Preset::TYPE_SLA_PRINT;
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        if ((print_tab && (
                (pr_tech == ptFFF && presets_type == Preset::TYPE_FFF_FILAMENT) ||
                (pr_tech == ptSLA && presets_type == Preset::TYPE_SLA_MATERIAL) )) ||
            (printer_tab && (
                (pr_tech == ptFFF && (presets_type == Preset::TYPE_FFF_PRINT || presets_type == Preset::TYPE_FFF_FILAMENT) ) ||
