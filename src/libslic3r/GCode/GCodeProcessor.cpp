...
void GCodeProcessor::TimeProcessor::post_process(const std::string& filename, std::vector<GCodeProcessorResult::MoveVertex>& moves, std::vector<size_t>& lines_ends)
{
        ...
        if (line.length() > 1) {
            ...
            if (export_remaining_time_enabled &&
                ...
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
                    ...
                    if (machine.enabled) {
                        ret += print_M73(machine,
                            (line == reserved_tag(ETags::First_Line_M73_Placeholder)) ? 0.f : machine.time,
                            (line == reserved_tag(ETags::First_Line_M73_Placeholder) && !machine.stop_times.empty()) ? machine.stop_times.front().elapsed_time : 0.f,
                            extra_lines_count);
                    }
                }
            }
    ...
    auto process_line_G1 = [
        ...
        time_in_minutes, format_time_float, time_in_last_minute,
        &self = std::as_const(*this),
        // Caches, to be modified
        &g1_times_cache_it, &print_M73,
        // String output
        &export_line]
...
void GCodeProcessor::apply_config(const DynamicPrintConfig& config)
{
    ...
    if (use_machine_limits) {// && (m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware || m_flavor == gcfRepRap)) {
        ...
        if (std::set<uint8_t>{gcfMarlinLegacy, gcfMarlinFirmware, gcfLerdge, gcfRepetier, gcfRepRap}.count(m_flavor) > 0) {
